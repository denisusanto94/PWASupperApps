<template>
  <div class="home-view">
    <div class="home-hero">
      <h1 class="home-title">PWASupperApps</h1>
      <p class="home-tagline">Pilih aplikasi untuk memulai</p>
    </div>
    <div class="app-buttons">
      <div @click="openWaBlaster" class="app-btn">
        <div class="app-btn-icon icon-wa">
        ...
        <span class="app-btn-title">WhatsApp Blaster</span>
      </div>


      <router-link to="/getlynkid" class="app-btn">
        <div class="app-btn-icon icon-link">
        ...
        <span class="app-btn-title">getlynk.id</span>
      </router-link>

      <router-link to="/instant-chat" class="app-btn">
    ...
    <!-- WhatsApp Blaster Modal -->
    <Transition name="modal-fade">
      <div v-if="showWaModal" class="reset-modal-overlay" @click.self="showWaModal = false">
        <div class="reset-modal-card">
           <div class="warn-icon icon-wa-modal">📱</div>
           <h3 class="modal-title">WhatsApp Blaster</h3>
           <p class="modal-body text-balance">Mulai sesi WhatsApp Blaster untuk mengirim pesan massal secara otomatis.</p>
           <div class="modal-actions">
              <button @click="showWaModal = false" class="btn-cancel">Batal</button>
              <button @click="performWaNavigation" class="btn-confirm-wa">Mulai Sekarang</button>
           </div>
        </div>
      </div>
    </Transition>
...
const performWaNavigation = () => {
  let session = localStorage.getItem('last_wa_session');
  if (!session) {
    session = 'user_' + Math.random().toString(36).slice(2, 9);
    localStorage.setItem('last_wa_session', session);
  }
  showWaModal.value = false;
  router.push({ path: '/wab-blaster', query: { session: session } });
};
